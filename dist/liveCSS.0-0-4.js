...
var helper = {
    ...
    // Holds a centralized unique index field
    unique : 0,
...
(function() {
    ...
    // =================================================================================================================
    var module = angular.module('LiveCss', [ 'LiveCssCore', 'LiveCssToolkit', 'LiveCssTimers', 'LiveCssDirectives' ]);
    ...
    // =================================================================================================================
    {
        module.provider('LiveCss', [ function( ) {
            ...
            return {
                ...
                // -----------------------------------------------------------------------------------------------------
                $get : [ 'LiveCssCore', 'LiveCssToolkit', 'LiveCssTimers', function( LiveCssCore, LiveCssToolkit, LiveCssTimers ) {
...
(function() {
    ...
    // =================================================================================================================
    {
        ...
        // Define Live CSS document extractor
        // -------------------------------------------------------------------------------------------------------------
        {
            /**
             ...
             */
            var LiveCssDocumentParser = function(syntax, attrs, element) {
                ...
                for (var i=0; i<sections.length; i++) if (sections[i].nodeName != '#comment') {
                    ...
                    if (docEl) {
                        ...
                        // Register document
                        documents.push(new LiveCssDocument(docEl, element));
                    }
        ...
        {
            /**
             * Parses Live CSS syntax processing
             * @param syntax Live CSS syntax
             * @param callbackFn Post-processing (clean-up) function
             * @returns Contained Live CSS syntax
             */
            var LiveCssSyntaxParser = function(syntax, callbackFn) {

                // Define syntax processors
                // -----------------------------------------------------------------------------------------------------

                // Recursively processes all XML nodes
                var xmlNodeProcessingFn = function(node) {
                    for (var i=0; i<node.childNodes.length; i++) {
                        if (node.childNodes[i].nodeName.toLowerCase() == '#text') {

                            /* TODO:
                                        Isolate " ($lcss-compilation-binding-start: ... :$lcss-compilation-binding-end) " replacements so that
                                        inside syntax will not interfere with selector parsing!!!
                            */
                            // Pre-process syntax
                            var syntax = node.childNodes[i].textContent;
                            syntax = syntax.replace(/{{/g, '($lcss-compilation-binding-start:').replace(/}}/g, ':$lcss-compilation-binding-end)');
                            syntax = syntax.replace(/\/\*/g, '<lcss-comment>').replace(/\*\//g, '</lcss-comment>');
                            syntax = syntax.replace(/{/g, '<lcss-block>').replace(/}/g, '</lcss-block>');

                            // Parse blocks
                            var cssHostElement = document.createElement('lcss');
                            cssHostElement.innerHTML = syntax;

                            // Process lcss syntax node
                            lcssNodeProcessingFn(cssHostElement);

                            // Get back parsed syntax
                            syntax = callbackFn( cssHostElement.innerHTML );

                            // Post-process syntax
                            syntax = syntax.replace(/\<lcss\-block\>/g, '{').replace(/\<\/lcss\-block\>/g, '}');
                            syntax = syntax.replace(/\<lcss\-comment\>/g, '/*').replace(/\<\/lcss\-comment\>/g, '*/');
                            syntax = syntax.replace(/\(\$lcss-compilation-binding-start\:/g, '{{').replace(/\:\$lcss-compilation-binding-end\)/g, '}}');

                            // Return processed syntax
                            node.childNodes[i].textContent = syntax;

                        } else if (node.childNodes[i].nodeName.toLowerCase() != 'lcss-comment') {
                            xmlNodeProcessingFn( node.childNodes[i] );
                        }
                    }
                };

                // Processes each LCSS syntax node
                var lcssNodeProcessingFn = function(node) {

                    // Find blocks
                    for (var i=0; i<node.childNodes.length; i++) {
                        if (node.childNodes[i].nodeName.toLowerCase() == 'lcss-block') {

                            // Check if has child blocks
                            var hasChildBlocks = false;
                            for (var j=0; j<node.childNodes[i].childNodes.length; j++) {
                                if (node.childNodes[i].childNodes[j].nodeName.toLowerCase() == 'lcss-block') { hasChildBlocks = true; break; }
                            }
                            if (hasChildBlocks) {

                                // Process node as host element
                                lcssNodeProcessingFn(node.childNodes[i]);

                            } else {

                                // Process this node's selector
                                if ((i > 0) && (node.childNodes[i - 1].nodeName.toLowerCase() == '#text')) {
                                    node.childNodes[i - 1].textContent = lcssSyntaxProcessingFn( node.childNodes[i - 1].textContent );
                                }
                ...
                // Processes LCSS selector syntax
                var lcssSyntaxProcessingFn = function(selectorSyntax) {
                    var parsedSelectorSyntax = selectorSyntax.split(','),
                        syntax = '';
                    for (var i=0; i<parsedSelectorSyntax.length; i++) {
                        var trimmed = parsedSelectorSyntax[i].trim();
                        syntax += (syntax.length ? ',' : '') +  parsedSelectorSyntax[i].replace(trimmed, '.$lcss-containment-class-placeholder ' + trimmed);
                    }
                    return syntax;
                }
                ...
                // Parse syntax
                var hostElement = document.createElement('lcss');
                hostElement.innerHTML = syntax;
                xmlNodeProcessingFn( hostElement );
                syntax = callbackFn( hostElement.innerHTML );
        ...
        // Define Live CSS syntax containment processor
        // -------------------------------------------------------------------------------------------------------------
        {
            /**
             * Parses Live CSS syntax containment within passed class name
             * @param syntax Live CSS syntax
             * @param className Container class name
             * @returns Contained Live CSS syntax
             */
            var LiveCssSyntaxContainmentParser = function(syntax, className) {
                return syntax.replace( /.\$lcss-containment-class-placeholder /g, (className ? '.' + className + ' ' : '') );
            }
        }
        ...
        // -------------------------------------------------------------------------------------------------------------
        {
             ...
             * @constructor
             */
            var LiveCssDocument = function(docEl, element) {
                ...
                var prv = {
                    ...
                    // -------------------------------------------------------------------------------------------------
                    attrs : {
                        // If document should be global (not contained)
                        global : false,
                        // Source URL or in code location
                        source : null,
                    ...
                    syntax : {
                        ...
                        loaded : null,
                        // Holds uncontained, parsed loaded version of document's syntax
                        global : null,
                        // Holds contained, parsed  loaded version of document's syntax
                        contained : null,
                    ...
                    initialize : function() {
                        ...
                        // Set document element's key attribute
                        // ---------------------------------------------------------------------------------------------
                        ...
                        // Contain CSS syntax and target host element
                        // ---------------------------------------------------------------------------------------------
                        var isGlobal = (prv.attrs['global'] ? pub.$scope.$eval( prv.attrs['global'] ) : false);
                        this.target.add( (isGlobal === false ? element : null)  );
                    ...
                    // Live CSS Document's embedding namespace
                    // -------------------------------------------------------------------------------------------------
                    target : {

                        // Unique class name used to contain live CSS syntax
                        uniqueClassName : null,

                        // Holds last processed syntax for change detection
                        processedSyntax : '',

                        // Host elements document is contained to
                        hostElements : { keys : [ ], targets : [ ] },

                        // Processes contained syntax and contains to host element
                        add : function(el) {

                            // Check if document previously contained
                            var isContained = prv.target.isContained();

                            // Check element
                            if (!el) { el = document.body; } else { el = (el.length ? el[0] : el); }

                            // Do global check
                            var currentEl = el, isGlobal = false;
                            while (currentEl) {
                                if ((el.tagName.toLowerCase() == 'head') || (el.tagName.toLowerCase() == 'html')) {
                                    // Set as global
                                    isGlobal = true; break;
                            ...
                            // Check if element isn't global
                            var targetEl = (isGlobal ? document.body : el);

                            // Contain live CSS syntax
                            if ((!pub.className) || (prv.target.processedSyntax != prv.syntax.loaded)) {

                                // Generate unique class name
                                if (!pub.className) prv.target.uniqueClassName = '__lcss-contained-' + pub.key;

                                // Parse (contain) syntax
                                var processedSyntax = LiveCssSyntaxParser( prv.syntax.loaded, prv.unhtmlize );
                                prv.syntax.contained = LiveCssSyntaxContainmentParser(processedSyntax, prv.target.uniqueClassName);
                                prv.syntax.global = LiveCssSyntaxContainmentParser(processedSyntax, null);
                                prv.target.processedSyntax = prv.syntax.loaded;

                            }

                            // Register target element
                            var previouslyAdded = false;
                            for (var i=0; i<prv.target.hostElements.keys.length; i++) if (prv.target.hostElements.keys[i] == el) { previouslyAdded = true; break; };
                            if (!previouslyAdded) {
                                prv.target.hostElements.keys.push( el );
                                prv.target.hostElements.targets.push( targetEl );
                                angular.element(targetEl).addClass( prv.target.uniqueClassName );
                            }

                            // If containment status changed, re-compile
                            if ((isContained !== null) && (isContained != prv.target.isContained())) prv.compile();

                        },

                        // Releses host elemnt from document's containment
                        remove : function(el) {
                            ...
                            // Un-register target element
                            for (var i=0; i<prv.target.hostElements.keys.length; i++) {
                                if (prv.target.hostElements.keys[i] == el) {
                                    // Get target
                                    var targetEl = prv.target.hostElements.targets[i];
                                    // Un-register
                                    prv.target.hostElements.keys.splice(i, 1);
                                    prv.target.hostElements.targets.splice(i, 1);
                                    // Check if target still registered (by other element)
                                    var stillRegistered = false;
                                    for (var j=i; j<prv.target.hostElements.keys.length; j++) if (prv.target.hostElements.targets[j] == targetEl) { stillRegistered = true; break; }
                                    if (!stillRegistered) angular.element(targetEl).removeClass( prv.target.uniqueClassName );
                                }
                            }
                        ...
                        // Checks if document is being contained to an element
                        isContained : function() {
                            for (var i=0; i<prv.target.hostElements.keys.length; i++) if (prv.target.hostElements.targets[i] == document.body) return false;
                            return (prv.target.hostElements.keys.length ? true : null);
                        }
                    ...
                    compile : function(callbackFn) {
                        // Check if syntax updated
                        if (prv.target.processedSyntax != prv.syntax.loaded) {
                            var processedSyntax = LiveCssSyntaxParser( prv.syntax.loaded, prv.unhtmlize );
                            prv.syntax.contained = LiveCssSyntaxContainmentParser(processedSyntax, prv.target.uniqueClassName);
                            prv.syntax.global = LiveCssSyntaxContainmentParser(processedSyntax, null);
                            prv.target.processedSyntax = prv.syntax.loaded;
                        ...
                        // Select syntax
                        var inputSyntax = (prv.target.isContained() ? prv.syntax.contained : prv.syntax.global);
                        if (inputSyntax) {
                            ...
                            prv.host.compilation.innerHTML = inputSyntax;
                ...
                var pub = {
                    ...
                    // -------------------------------------------------------------------------------------------------
                    key : ((new Date()).getTime() + 1000000 * (helper.unique++)).toString(16),
                    ...
                    /**
                     * Elements targeting (Embedding) fucntionality
                     */
                    target : {
                        /**
                         * Embeds live CSS document's syntax into target element
                         */
                        add : prv.target.add,
                        /**
                         * Releses target element from live CSS document's syntax
                         */
                        remove : prv.target.remove
                    },
                ...
                $get : [        '$compile', '$interpolate', '$rootScope', '$q', '$http', '$timeout',
                    ...
                    // -------------------------------------------------------------------------------------------------
                    {
                        ...
                        var pub = {
                            ...
                            /**
                             * Fetches Live CSS document by its defined key or source
                             * @param key Live CSS document key or source
                             * @returns {*} Selected Live CSS document
                             */
                            fetch : function(key) {
                                if (key && registry.documents[key]) {
                                    // Fetch Live CSS document by key
                                    return registry.documents[key];
                                } else {
                                    // Try fetching by source
                                    for (var i in registry.documents) if ( key && registry.documents[i].attrs.source && registry.documents[i].attrs.source.toLowerCase().trim() == key.toLowerCase().trim() ) return registry.documents[i];
                             ...
                             */
                            import : function(syntax, attrs, element) {
                                // Parse live CSS documents
                                var docs = LiveCssDocumentParser(syntax, (attrs ? attrs : { source : null }), element);
                                // Return live CSS documents
                                return docs;
                            },
                            /**
                             ...
                             */
                            importUrl : function(url, element) {
                                // Initialize promise
                                var deferredResult = $q.defer();
                                // Get file from url
                                $http.get(url).then(
                                    (function(element) { return function(response) {
                                        // Check url
                                        var isLCSS = (url.split('.')[ url.split('.').length - 1 ].toLowerCase().trim() == 'lcss');
                                        // Process syntax
                                        var docs = pub.import(response.data, { source : url }, (isLCSS ? element : null));
...
// Live CSS Timers module
// =====================================================================================================================
(function() {

    // Initialize Live CSS timers module
    // =================================================================================================================
    var module = angular.module('LiveCssTimers', [ 'LiveCssCore' ]);

    // Live CSS timers private namespace
    // -----------------------------------------------------------------------------------------------------------------
    var registry = { };

    // Initialize Live CSS timers provider
    // =================================================================================================================
    {
        module.provider('LiveCssTimers', [ function( ) {

            // Initialize and return Live CSS timers provider's definition
            // ---------------------------------------------------------------------------------------------------------
            return {

                // Returns Live CSS timers provider instance
                // -----------------------------------------------------------------------------------------------------
                $get : [ 'LiveCssCore', '$timeout', function( LiveCssCore, $timeout ) {

                    // Initialize Live CSS timers provider's internals
                    // -------------------------------------------------------------------------------------------------
                    {

                        // Live CSS timers public namespace
                        // ---------------------------------------------------------------------------------------------
                        var pub = { };
                    ...
                    // Return public (exposed) namespace
                    // -------------------------------------------------------------------------------------------------
                    services.LiveCssTimers = { pub : pub, prv : registry };
                    return pub;
...
(function() {
    ...
    // Initialize Live CSS directives module
    // =================================================================================================================
    var module = angular.module('LiveCssDirectives', [ 'LiveCssCore', 'LiveCssToolkit', 'LiveCssTimers' ]);
    ...
    // =================================================================================================================
    {
        ...
        // -------------------------------------------------------------------------------------------------------------
        {
                ...
                if (registry[key]) {
                    ...
                    for (var i=0; i < registry[key].elements.length; i++) {
                        if (element[0] == registry[key].elements[i]) {
                            // Remove element as target
                            for (var j=0; j<registry[key].docs.length; j++) if (element.length && element[0].parentNode) registry[key].docs[j].target.remove(element[0].parentNode);
                            // Un-register element
            ...
            // Define LiveCSS include ('lcss') directive
            // ---------------------------------------------------------------------------------------------------------
            module.directive('lcss', [ 'LiveCssCore', 'LiveCssToolkit', function( LiveCssCore, LiveCssToolkit ) {

                // Define a hash function (Java syntax)
                var hashFn = function(str) {
                    var hash = 0;
                    if (str.length == 0) return hash;
                    for (var i = 0; i < str.length; i++) {
                        var char = str.charCodeAt(i); hash = ((hash<<5)-hash)+char; hash = hash & hash;
                    }
                    return hash;
                };
                ...
                return {
                    ...
                    link : function($scope, element, attrs) {
                        ...
                        // Include from element content
                        if (element && element.length && element[0].innerText && element[0].innerText.trim().length > 0) {

                            // Import syntax
                            var lcssSyntax = element[0].innerText.trim(),
                                lcssHash = hashFn(lcssSyntax);

                            // Check if previously loaded
                            if (!registry[lcssHash]) {

                                // Load live CSS
                                var attrs = { source : 'inline[' + lcssHash + ']' },
                                    docs = LiveCssCore.import(lcssSyntax, attrs, ((!attrs['global'] || !$scope.$eval(attrs['global'])) && element[0].parentNode ? element[0].parentNode : null));

                                // Clear syntax from element
                                element[0].innerText = '<!-- LiveCSS syntax imported -->';

                                // Register elements
                                directiveAddedFn(lcssHash, docs, element);

                                // Handle element removal
                                element.on('$destroy', function() { directiveRemovedFn(lcssHash, element ); });
                                ...
                                var docs = directiveAddedFn(lcssHash, [ ], element);

                                // Contain to element
                                for (var i=0; i<docs.docs.length; i++) docs.docs[i].target.add((element[0].parentNode ? element[0].parentNode : null));
                        ...
                        $scope.$watch(
                            function() {
                                ...
                                var src = attrs['src'],
                                    global = attrs['global'];
                                try { src = $scope.$eval(src); } catch (ex) { }
                                try { global = $scope.$eval(global); } catch (ex) { }
                                return { src : src, global : global };
                            },
                            function(newVal, oldVal) {
                                ...
                                // Get source url
                                var url = newVal.src,
                                    isGlobal = newVal.global;

                                // Check if previously loaded
                                if (!registry[url]) {

                                    // Load live CSS
                                    LiveCssCore.importUrl(url, (!isGlobal && element[0].parentNode ? element[0].parentNode : null)).then(function(docs) {
                                    ...
                                    // Register element to previously loaded LiveCSS
                                    var docs = directiveAddedFn(url, [ ], element);
