        ...
        var LiveCssElementWatcher = function(key, element, imports) {
            ...
            var propertiesAssemblyFn = function() {
                    ...
                    for (var i=0; i<propertyExtensions.length; i++) {
                        ...
                        // Time action
                        if (services.LiveCssDebugger) var timer = services.LiveCssDebugger.stats.domains.get( [ 'extension', 'lcss-element', 'polling', key, uniqueKey, 'watch' ], extension.getAdditionalKeys() ).time();
            ...
            // Check if newly created element scope
            if (newScope) {
            ...
            var watcher = registry.element.elementScope.$watch(
                ...
                function(newVal, oldVal) {
                    ...
                    if (newVal) {
                        ...
                        // Time action
                        if (services.LiveCssDebugger) var timer = services.LiveCssDebugger.stats.domains.get( [ 'extension', 'lcss-element', 'polling', key, uniqueKey, 'apply' ] ).time();
            ...
            return {
                ...
                $get : [ 'LiveCssCore', function( LiveCssCore ) {
                    ...
                    // -------------------------------------------------------------------------------------------------
                    {
                        ...
                        var extension = LiveCssCore.extend('mouse', function(domain) {
                            domain.position = {
                        ...
                        extension.setAdditionalKeys( [ 'position' ] );
                        ...
                        angular.element(document.body).on('mousemove', mousePositionApplyFnFactory(extension));
                        angular.element(document.body).on('click', mousePositionApplyFnFactory(extension));
                        angular.element(document.body).on('touchstart', mousePositionApplyFnFactory(extension));
                    }
